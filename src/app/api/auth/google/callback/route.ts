...
// 구글 신규 가입 시 phoneNumber, 약관 동의 등 추가 정보 입력 필요
...
export async function GET(req: NextRequest) {
  ...
  try {
    // 구글 access_token 발급
    const googleTokenData = await getGoogleToken(code);

    // 구글 사용자 정보 조회
    const googleUser = await getGoogleUser(googleTokenData.access_token);

    const { googleId, email, nickname } = extractGoogleUserInfo(googleUser);

    // googleId 기준으로 기존 사용자 조회 (SocialAccount → User)
    ...
    if (socialAccount) {
      ...
      // 신규 구글 사용자 → 계정 생성
      // 닉네임 중복 시 googleId 전체로 대체 (googleId는 전역 고유값이므로 충돌 없음)
      ...
      if (existingNick) {
        finalNickname = `google_${googleId}`;
      }
    ...
    // 우리 서비스 JWT 발급 (Google access_token은 포함하지 않음)
