...
// 카카오 신규 가입 시 phoneNumber, 약관 동의 등 추가 정보 입력 필요
const ADDITIONAL_INFO_REDIRECT = "/signup/additional-info?provider=kakao";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // 카카오 사용자 정보 조회
    const kakaoUser = await getKakaoUser(kakaoTokenData.access_token);

    if (!kakaoUser.id) {
      console.error("[auth.kakao.callback] kakaoId가 없습니다.");
      return NextResponse.redirect(`${baseUrl}${LOGIN_FAILURE_REDIRECT}`);
    }

    const { kakaoId, email, nickname } = extractKakaoUserInfo(kakaoUser);

    // kakaoId 기준으로 기존 사용자 조회 (SocialAccount → User)
    ...
    if (socialAccount) {
      ...
      // 신규 카카오 사용자 → 계정 생성
      // 닉네임 중복 시 kakaoId 전체로 대체 (kakaoId는 전역 고유값이므로 충돌 없음)
    ...
    // 우리 서비스 JWT 발급 (Kakao access_token은 포함하지 않음)
